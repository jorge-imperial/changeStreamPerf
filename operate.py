...
def updates(uri, file_name, coll_name, count, r, output_filename):
...
def inserts(uri, file_name, coll_name, count, r, output_filename):
    client = MongoClient(uri)
    with open(output_filename, 'wt') as outf:
        # read json in
        with open(file_name, 'rt') as f:
...
def deletes(uri, file_name, coll_name, count, r, output_filename):
    client = MongoClient(uri)
    # read json in
    with open(output_filename, 'wt') as outf:
        with open(file_name, 'rt') as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--out_update', default='update_ops.json')
    parser.add_argument('--out_insert', default='insert_ops.json')
    parser.add_argument('--out_delete', default='delete_ops.json')
